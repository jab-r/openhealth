...
<h2>Patient:
<span class="patient-name"><xsl:value-of select="$patname" /></span>-<span class="patient-id"><xsl:value-of select="ch:clinical.header/ch:patient/ch:id[@type='patient.identifier']" /></span>
</h2>
<h3>Procedure: <span class="procedure"><xsl:value-of select="o:clinical.body/o:procedure" /></span></h3>
<p>CPT: <span class="CPT"><xsl:for-each select="ch:clinical.header/ch:codes/ch:coded.value[@code.system.name='CPT']"><xsl:if test="position()>1">,</xsl:if><xsl:value-of select="."/></xsl:for-each></span></p>
<h3>Preoperative Diagnosis: <span class="preop-diagnosis"><xsl:value-of select="o:clinical.body/o:preoperative.diagnosis" /></span></h3>
<h3>Postoperative Diagnosis: <span class="postop-diagnosis"><xsl:value-of select="o:clinical.body/o:postoperative.diagnosis" /></span></h3>
