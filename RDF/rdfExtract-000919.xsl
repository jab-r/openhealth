...
<rdf:Statement>
	<rdf:predicate><xsl:call-template name="QNameToURI"><xsl:with-param name="namespace-name" select="$predicate-namespace-uri"/><xsl:with-param name="local-name" select="$predicate-local-name"/></xsl:call-template></rdf:predicate>
	<rdf:subject><xsl:value-of select="$subject" /></rdf:subject>
	<rdf:object rdf:type="{$otype}"><xsl:value-of select="string($object)" /></rdf:object>
</rdf:Statement>
