<?xml version="1.0" ?>
<!--
	http://www.openhealth.org/RDF/RDFSurfaceSyntax.xsl
...
<!ENTITY rdfns "http://www.openhealth.org/RDF/RDFSurfaceSyntax#">
]>
...
>
<xsl:import href="rdfn.xsl"/>
<xsl:output method="xml" indent="yes"/>
  <xsl:variable name='rdfNS' select='"&rdfns;"'/>
  ...
  <xsl:variable name='uriReferenceType' select='"http://www.w3.org/2001/XMLSchema#uriReference"'/>
  <xsl:variable name='stringType' select='"http://www.w3.org/2001/XMLSchema#string"'/>
...
<!--                                                                         -->
<xsl:template match="rdf:Statement" mode="objects">
<xsl:if test="$trace">[TRACE rdf:Statement]</xsl:if>
	<xsl:copy>
		<xsl:copy-of select="@*"/>
		<xsl:apply-templates select="node()" mode="copy"/>
	</xsl:copy>
...
<xsl:template match="rdf:Description[@rdf:about]" mode="objects">
...
<xsl:if test="$trace">[TRACE @* <xsl:value-of select="name()" /> property-attributes]</xsl:if>
