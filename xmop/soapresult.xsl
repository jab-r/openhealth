...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				...
				version="1.0">
<xsl:param name="res"/>
<xsl:template match="/"><xsl:apply-templates select="/SOAP:Envelope"/></xsl:template>
...
<xsl:template match="*"><xsl:for-each select="*"><xsl:copy><return><xsl:value-of select="/SOAP:Envelope/result" /></return></xsl:copy>
