...
<xsl:template match="/">
<xsl:apply-templates select="/methodCall" />
</xsl:template>
<xsl:template match="/methodCall">
	<xsl:variable name="uri" select="methodName" />
	<xsl:variable name="progid" select="substring-before(substring-after($uri,'http://www.grovelogic.com/XMLRPC/ProgID/'),'#')" />
	<xsl:variable name="end" select="count(params/param)" />
	var obj = Server.CreateObject(&quot;<xsl:value-of select="translate($progid,';&quot;-+=,','..___ ')" />&quot;);
	res = obj.<xsl:value-of select="substring-after($uri,'#')" />(<xsl:apply-templates select="params"/>);
</xsl:template>
<xsl:template match="params">
	<xsl:variable name="end" select="count(child::*)" />
	<xsl:for-each select="*">'<xsl:value-of select="."/>'<xsl:if test="position() != $end">,</xsl:if></xsl:for-each>
</xsl:template>
</xsl:stylesheet>
